...
// this is the function that Dhen should code
function generateRealisticValueFor(dataType) {
    switch (dataType) {
        case 'HeartRate':
            return (Math.random() * 4.27 /* population heart rate variance */) + 127 /* average heart rate */;
            break;
        case 'BodyWeight':
            break;
        case 'BodyHeight':
...
function generateObservationFor(email, dataType, dateRange) {
    ...
    IndividualUser.findOne({email: email}, (err, user) => {
        ...
        CompanyAssociation.findOne({user: user._id}, (err, companyAssociations) => {
            ...
            observation.value = generateRealisticValueFor(dataType);
...
for (let i = 0; i < 50 /* no of times to run */; i++) {
